...
void iothub_client_sample_amqp_websockets_shared_run(void)
{
    ...
    else 
    {
        ...
        else
        {
            ...
            if ((iotHubClientHandle1 = IoTHubClientCore_LL_CreateWithTransport(&config1)) == NULL)
            {
                (void)printf("ERROR: iotHubClientHandle1 is NULL!\r\n");
            }
            else if ((iotHubClientHandle2 = IoTHubClientCore_LL_CreateWithTransport(&config2)) == NULL)
            {
                (void)printf("ERROR: iotHubClientHandle1 is NULL!\r\n");
            }
            else
            {
                bool traceOn = true;
                IoTHubClientCore_LL_SetOption(iotHubClientHandle1, OPTION_LOG_TRACE, &traceOn);

    #ifdef SET_TRUSTED_CERT_IN_SAMPLES
                // For mbed add the certificate information
                if (IoTHubClientCore_LL_SetOption(iotHubClientHandle1, OPTION_TRUSTED_CERT, certificates) != IOTHUB_CLIENT_OK)
                {
                ...
                else if (IoTHubClientCore_LL_SetMessageCallback(iotHubClientHandle1, ReceiveMessageCallback, &receiveContext1) != IOTHUB_CLIENT_OK)
                {
                    (void)printf("ERROR: IoTHubClientCore_SetMessageCallback for device 1..........FAILED!\r\n");
                }
                else if (IoTHubClientCore_LL_SetMessageCallback(iotHubClientHandle2, ReceiveMessageCallback, &receiveContext2) != IOTHUB_CLIENT_OK)
                {
                    (void)printf("ERROR: IoTHubClientCore_SetMessageCallback for device 2..........FAILED!\r\n");
                }
                else
                {
                    (void)printf("IoTHubClientCore_SetMessageCallback...successful.\r\n");
                    ...
                    do
                    {
                        ...
                        {
                            if (IoTHubClientCore_LL_SendEventAsync(iotHubClientHandle1, messages_device1[iterator].messageHandle, SendConfirmationCallback, &messages_device1[iterator]) != IOTHUB_CLIENT_OK)
                            {
                                (void)printf("ERROR: IoTHubClientCore_SendEventAsync for device 1..........FAILED!\r\n");
                            }
                            else if (IoTHubClientCore_LL_SendEventAsync(iotHubClientHandle2, messages_device2[iterator].messageHandle, SendConfirmationCallback, &messages_device2[iterator]) != IOTHUB_CLIENT_OK)
                            {
                                (void)printf("ERROR: IoTHubClientCore_SendEventAsync for device 2..........FAILED!\r\n");
                            }
                            else
                            {
                                (void)printf("IoTHubClientCore_SendEventAsync accepted data for transmission to IoT Hub.\r\n");
                            }
                        }

                        IoTHubClientCore_LL_DoWork(iotHubClientHandle1);
                        IoTHubClientCore_LL_DoWork(iotHubClientHandle2);
                        ThreadAPI_Sleep(1);
                ...
                IoTHubClientCore_LL_Destroy(iotHubClientHandle1);
                IoTHubClientCore_LL_Destroy(iotHubClientHandle2);
            }
